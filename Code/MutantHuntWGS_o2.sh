...
#updated for o2 by Tyrone Lee (4/3/23)
...
then
	...
	mkdir "$OUTPUT_FILE"/QualiMap
	mkdir "$OUTPUT_FILE"/zero-read_Coverage
    
    	# Get current date and time
    	now=$(date +"%Y-%m-%d %H:%M:%S")

    	# Write inout params to script in txt file
    	echo "Script 'MutantHuntWGS_o2' ran on $now:" >> "$OUTPUT_FILE"/params.txt
    	args="$@"
      		for arg in $args
	do 
            	echo $arg >> "$OUTPUT_FILE"/params.txt
        done
...
then
	...
	do
		...
		then
			...
			java -jar $SNPEFF/snpEff.jar ann -dataDir /n/shared_db/snpEff/4.3g/ -v Saccharomyces_cerevisiae "$VCF_FILE" > "$OUTPUT_FILE"/SNPeff_Output/"$VCF_NAME"/SNPeff_Annotations.vcf
	...
	################################################
	###   MODULE 6: QualiMap report generation   ###
	################################################

	echo -e "\n\n\n\n\n\n"Generating QualiMap reports  "\n"

	for BAM_FILE in `ls "$OUTPUT_FILE"/BAM/*_sorted.bam`
	do
		NAME_PREFIX=`echo "$BAM_FILE" | awk -F "/" '{print $(NF)}' | awk -F "_" '{print  $1}'`
		echo -e command: qualimap bamqc -bam $BAM_FILE --outdir  "$OUTPUT_FILE"/QualiMap/ --outfile "$NAME_PREFIX"_stats.pdf --outformat PDF --paint-chromosome-limits
		qualimap bamqc -bam "$BAM_FILE" --outdir  "$OUTPUT_FILE"/QualiMap/ --outfile "$NAME_PREFIX"_stats.pdf --outformat PDF --paint-chromosome-limits
	done

	##############################################################################
	###   MODULE 7: Bedtools genomeCoverageBed gap/zero-read coverage report   ###
	##############################################################################

	echo -e "\n\n\n\n\n\n"Generating gap/zero-read reports  "\n"
	for BAM_FILE in `ls "$OUTPUT_FILE"/BAM/*_sorted.bam`
	do
		NAME_PREFIX=`echo "$BAM_FILE" | awk -F "/" '{print $(NF)}' | awk -F "_" '{print  $1}'`
		bedtools genomecov -ibam "$BAM_FILE" -bga | awk '$4 < 1' > "$OUTPUT_FILE"/zero-read_Coverage/"$NAME_PREFIX"_0genomeCov.txt
		echo  -e Saved to "$OUTPUT_FILE"/zero-read_Coverage/"$NAME_PREFIX"_0genomeCov.txt 
